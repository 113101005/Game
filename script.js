document.addEventListener('DOMContentLoaded', () => {
    ...
    const nextLevelBtn = document.getElementById('next-level-btn'); // 這個似乎沒有在 HTML 中使用
    const homeBtn = document.getElementById('home-btn'); // 獲取回首頁按鈕
    ...
    const questionNumberDisplay = document.getElementById('question-number-display'); // 獲取題數顯示元素
...
const leaderboardSection = document.getElementById('leaderboard-section'); // 確保有這行
    ...
    let totalQuestionsInGame = 0; // 儲存當前遊戲的總題數
    let answeredQuestions = []; // 新增：儲存每題的答題結果和資訊
    ...
    function showQuestion(question) {
        ...
        question.options.forEach(option => {
            ...
            button.addEventListener('click', () => selectOption(button, option.text, option.isCorrect, question)); // 傳遞選中的文字
    ...
    function selectOption(selectedButton, selectedOptionText, isCorrect, currentQ) {
        // 選項選擇後，禁用所有選項按鈕
        ...
        answeredQuestions.push({
            ...
            selectedOption: selectedOptionText // 記錄玩家選擇的選項文字
        });
    }
    ...
    function startGame() {
        ...
        homeBtn.classList.add('hidden'); // 遊戲開始時隱藏回首頁按鈕
        currentQuestionIndex = 0;
        score = 0;
        timer = 0;
        answeredQuestions = []; // 重置已回答題目
        timerDisplay.textContent = timer;
        scoreDisplay.textContent = score;
        submissionFeedback.textContent = ''; // 清除提交分數的回饋訊息
        playerNameInput.value = ''; // 清空玩家名稱輸入框
leaderboardSection.classList.add('hidden'); // 新增這行
        // 隱藏回顧區塊和按鈕，準備開始新遊戲
        ...
        totalQuestionsInGame = questionsForCurrentGame.length; // 設定當前遊戲的總題數
    ...
    function endGame() {
      ...
      leaderboardSection.classList.remove('hidden'); // <-- 新增這行
        loadLeaderboard(); // 遊戲結束時載入排行榜

        // 顯示回顧按鈕和回首頁按鈕
        viewReviewBtn.classList.remove('hidden');
        homeBtn.classList.remove('hidden'); // 遊戲結束時顯示回首頁
        // 不自動顯示回顧內容，等待點擊按鈕
        reviewContainer.classList.add('hidden');
    }
    ...
    function updateQuestionNumberDisplay() {
        if (questionNumberDisplay) {
            questionNumberDisplay.textContent = `${currentQuestionIndex + 1} / ${totalQuestionsInGame}`;
        }
    }
    ...
    function displayReview() {
        reviewQuestionsList.innerHTML = ''; // 清空之前的回顧內容

        if (answeredQuestions.length === 0) {
            reviewQuestionsList.innerHTML = '<p>本次遊戲沒有答題紀錄。</p>';
            return;
        }

        answeredQuestions.forEach((item, index) => {
            const questionItem = document.createElement('div');
            questionItem.classList.add('review-item'); // 可以為回顧項目新增 CSS 樣式

            let feedbackClass = item.answeredCorrectly ? 'review-correct' : 'review-incorrect'; // 新增 CSS 類別來區分對錯

            // 找到正確答案的文字
            const correctAnswerText = item.question.options.find(opt => opt.isCorrect).text;

            questionItem.innerHTML = `
                <p><strong>第 ${index + 1} 題:</strong> ${item.question.question}</p>
                <p class="${feedbackClass}">你的回答: ${item.selectedOption} (${item.answeredCorrectly ? '正確' : '錯誤'})</p>
                ${!item.answeredCorrectly ? `<p>正確答案: ${correctAnswerText}</p>` : ''}
                <div class="explanation-container">
                    <h3>解釋與例句：</h3>
                    <p>${item.question.explanation}</p>
                    <p>例句：${item.question.example}</p>
                </div>
                <hr>
            `;
    ...
    restartGameButton.addEventListener('click', () => { // 重新開始時也隱藏回顧區塊和按鈕
        reviewContainer.classList.add('hidden');
        viewReviewBtn.classList.add('hidden');
        startGame();
    });
    homeBtn.addEventListener('click', () => { // 回首頁按鈕的事件監聽器
        ...
        questionNumberDisplay.textContent = '0 / 0'; // 初始載入時顯示 0/0
        playerNameInput.value = '';
        submissionFeedback.textContent = '';
        submitScoreButton.disabled = false; // 確保按鈕可再次使用
        answeredQuestions = []; // 回首頁時也清空已回答題目
        // 隱藏回顧區塊和按鈕
        reviewContainer.classList.add('hidden');
        viewReviewBtn.classList.add('hidden');
    ...
    // 新增：回顧題目按鈕的事件監聽器
    viewReviewBtn.addEventListener('click', displayReview);
